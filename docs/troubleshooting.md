...
| `SmartTokenError`: "Certificado expirado" / "Certificado ainda não válido"                                                                              | `certificatePem`/`serverTrustAnchor` fora do período de validade (RF-14)                                                                                                         | Gere/obtenha um certificado válido; confira `openssl x509 -noout -dates -in certificado.pem`                                                                                       |
| `SigningError`: "Falha ao assinar dados..." mesmo com chave/certificado corretos                                                                        | Certificado não corresponde à chave privada (RF-15)                                                                                                                              | Compare _modulus_: `openssl x509 -noout -modulus -in cert.pem \| openssl md5` vs `openssl rsa -noout -modulus -in key.pem \| openssl md5`                                          |
